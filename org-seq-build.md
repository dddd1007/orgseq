...
## 与本仓库 `org-seq` 的对应关系（读文档时先看这段）

本仓库已在上述通用方案上做了整包落地，差异点如下，避免与文中泛化示例混淆：

- **模块拆分**: `lisp/init-org.el` 只管 Org 基座与局部键；**GTD 面板、agenda 自定义命令与状态机**在 `lisp/init-gtd.el`；**org-supertag、transclusion、org-ql 扩展**在 `lisp/init-pkm.el`；**org-roam + org-node + org-mem** 在 `lisp/init-roam.el`。加载顺序见仓库根目录 `CLAUDE.md`。
- **性能栈**: 使用 **org-node** 与 **org-mem** 做异步索引；`org-mem-roamy-db-mode` 可向 org-roam 的 SQLite DB 供数，便于 org-roam-ui。**不再使用 md-roam**，笔记以 **Org** 为主。
- **数据层**: **org-supertag** 作为 Tana 式结构化标签核心（安装源 GitHub，非 MELPA）；首次使用需在本配置中执行 `M-x supertag-sync-full-initialize`（或 `SPC n s R`）。
- **日常操作**: 键位与流程以 `README.md`、`WORKFLOW.md` 为准；本文件仍作原理与排错参考。
...
└── lisp/
    ...
    ├── init-markdown.el   ; Markdown（可选）
    ├── init-org.el        ; Org 基座 + evil-org + 局部 leader
    ├── init-roam.el       ; org-roam + org-node/org-mem
    ├── init-gtd.el        ; GTD 面板与 agenda
    ├── init-pkm.el        ; org-supertag + transclusion + org-ql
    ├── init-ai.el         ; gptel / ob-gptel
    ├── init-dashboard.el  ; 启动页
    ├── init-workspace.el  ; 布局
    └── init-evil.el       ; Evil + SPC leader（最后加载）
```
...
### org-supertag -- 结构化标签 / 数据层（本仓库核心之一）

org-supertag (https://github.com/yibie/org-supertag) 提供类似 Tana 的 supertag：**带字段模式的标签**、查询、表格视图、看板、与 org-id 协作。**不在 MELPA**，用 `package-vc-install` 从 GitHub 安装。上游仍在快速迭代；本仓库将其作为 **PKM 数据层**，与 org-roam（图谱）和 org-node/org-mem（索引性能）分层配合。

**在本仓库中的要点**（详见 `lisp/init-pkm.el`、`CLAUDE.md`）：

- 同步目录与 org-roam 一致（默认 `~/NoteHQ/Roam/`）。
- 启动阶段对同步做了 **延迟/idle**，减轻冷启动阻塞；捕获完成后可通过 hook 触发 `supertag-sync-check-now`。
- 首次或重建索引：`M-x supertag-sync-full-initialize`。

示意安装（实际配置以仓库内 `use-package` 为准）:

```elisp
(package-vc-install "https://github.com/yibie/org-supertag")
```

与 org-roam 互补：roam 管 **链接与节点身份**，supertag 管 **结构化元数据与查询**。
...
| org-node | MELPA | 节点索引/加速 | 与 org-mem 联用 |
| org-mem | MELPA | 异步扫描、agenda 文件列表等 | 见 `init-roam.el` |
| org-transclusion | GNU ELPA | 内容嵌入 | 无 |
| org-ql | MELPA | 查询语言 | dash 版本；GTD 面板在用 |
| org-supertag | GitHub | 结构化标签 / 数据层 | 非 MELPA；首次 full init |
...
org-roam 与 Obsidian 等工具的核心差异不在功能而在范式. org-roam 使用 **带 org-id 的链接**，重命名文件不易断链；节点可以是文件级或标题级. Obsidian 的嵌入大致对应 org-transclusion；**本仓库不做 md-roam**，PKM 主路径是 **Org + org-roam + org-supertag**. 若需双开 Obsidian，仅适合作为目录浏览器或阅读辅助，**Roam 与 supertag 闭环应在 Emacs 内用 `.org` 完成**。

**迁移建议**：仍以 org-roam 为主库，重要内容手动迁入 Org 并重建链接；批量自动化转换往往得不偿失。知识管理系统的价值在于使用过程中的思考, 而非数据本身的搬运.