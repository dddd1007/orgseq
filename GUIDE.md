...
│   ├── lit/                   ← 文献笔记
│   ├── concepts/              ← 概念笔记
│   │   ├── purpose.org        ← AI 上下文：知识库目标
│   │   ├── schema.org         ← AI 上下文：笔记结构规则
│   │   └── overview.org       ← AI 生成的知识库概览
│   └── *.org                  ← 其他笔记
├── .orgseq/                   ← 个性化配置（类似 .vscode/）
│   └── ai-config.org          ← AI 服务配置（后端、模型）
└── *.org                      ← 非 Roam 笔记（也参与 GTD 扫描）
```
...
2. 创建 `~/NoteHQ/` 目录结构
3. 创建 `~/NoteHQ/.orgseq/ai-config.org` 配置文件
4. 显示 Dashboard 启动界面
...
;; 初始化 org-supertag 索引（如果你要用结构化标签功能）
...
|------|------|------|---------|
| `d` | 默认 | 通用笔记 | 无特殊结构 |
| `l` | 文献笔记 | 读书/论文笔记 | Core Ideas / Methodology / Relevance / Notes |
| `c` | 概念笔记 | 定义和解释 | Definition / Related Concepts / Notes |
| `f` | 闪念笔记 | 快速捕获想法 | 立即保存，不打开编辑器 |

每条笔记自动获得一个唯一 ID（时间戳格式），存储在 `~/NoteHQ/Roam/` 下。
...
org-supertag 把 Org 文件中的 `#标签` 变成数据库表格。例如给一个笔记加上 `#book` 标签，就可以自动出现"作者"、"评分"等字段。

```
SPC n s a      → 给当前节点添加 supertag
SPC n s v      → 以表格视图查看标签数据
SPC n s s      → 搜索 supertag 数据库
SPC n s k      → 以看板（Kanban）视图查看
SPC n s c      → 通过 supertag 捕获新条目
SPC n s n      → 创建新节点
```

在 Org buffer 中也可以用 local leader 快速访问：
...
```
SPC n s R      → 或 M-x supertag-sync-full-initialize
```

### 8.9 Transclusion（内容嵌入）
...
虽然 org-seq 以 Org-mode 为核心，但完整支持 Markdown 编辑。打开 `.md` 文件会自动启用：
