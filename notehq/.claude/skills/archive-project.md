...
description: Move a completed Outputs/ project to Archives/
user_invocable: true
---

The user wants to archive a completed or paused project from Outputs/ to Archives/.
...
3. Determine the archive name. Convention: `Archives/YYYY-project-name/`
   - Use the current year prefix for dating
   - Keep the original directory name

4. Check for any unresolved tasks in the project:
   ```bash
   grep -rn "^\*.*\(TODO\|NEXT\|IN-PROGRESS\|WAITING\)" Outputs/project-name/ 2>/dev/null
   ```
   If found, warn the user and ask whether to proceed or resolve them first.

5. Move the project:
   ```bash
   mv Outputs/project-name Archives/$(date +%Y)-project-name
   ```

6. Report what was moved and any remaining references:
   ```bash
   grep -rn "project-name" Roam/ --include="*.org" 2>/dev/null | head -10
   ```
...
- After archiving, the project no longer appears in GTD agenda views (Archives/ is excluded from agenda scan)
- The user can always un-archive by moving back to Outputs/
