...
org-seq 现在把 `pyim` 和 `sis` 组合在一起使用：`pyim` 负责真正的中文输入，`sis` 负责在合适的时机帮你回到英文状态。

- `C-\`：手动切换输入法
- `M-j`：把光标前的拼音强制转成中文
- 离开 Evil Insert、进入 minibuffer、输入 `C-c` / `C-x` / `C-h` 这类前缀键时，会自动回英文
- 回到某个 buffer 时，会尽量恢复那个 buffer 之前的输入状态

这意味着写中文笔记时基本可以连续输入；而执行命令、搜索、走键位链时，不必频繁手动切换输入法。
