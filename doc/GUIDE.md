...
1. **笔记用 `.org` 格式存在 `~/NoteHQ/Roam/` 下**——不是数据库，不是云端，而是人眼可读的纯文本文件。二十年后你的 Emacs 没了，这些 `.org` 文件依然能被任何文本编辑器打开。
...
- `~/NoteHQ/Roam/purpose.org` 描述你的知识库目标和研究方向。例如"我是研究认知科学的博士生，当前关注元认知与学习迁移"。
- `~/NoteHQ/Roam/schema.org` 描述你的笔记类型约定和 tag 体系。例如"reading tag 记录文献，field 有 authors / year / topic"。
...
- **dirvish-side**（左）：基于 dired 的现代文件管理器侧边栏。支持 nerd-icons 图标、git 状态、文件大小、子目录展开、hover 预览。所有 dired 键位都能直接用。按 `a` 弹出 quick-access 菜单一键跳转到 NoteHQ 各层级。
- **主编辑区**（中）：你的笔记/代码
...
| `my/roam-dir` | 原子笔记层（org-roam + supertag + AI context 全部指向此处） | `~/NoteHQ/Roam/` |
...
| 修改内容 | 热重载方式 |
|---------|-----------|
| supertag schema（`~/NoteHQ/Roam/supertag-schema.el`） | `SPC n m T`（`my/reload-supertag-schema`） |
